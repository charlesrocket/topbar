...
void WifiNetwork::connectWithPsk(const QString &psk) {
    ...
    if (this->bSecurity != WifiSecurityType::WpaPsk
        && this->bSecurity != WifiSecurityType::Wpa2Psk
        && this->bSecurity != WifiSecurityType::Sae) {
        qCCritical(logWifiNetwork)
            << this << "has the wrong security type for a PSK.";
        return;
    }
...
WifiDevice::WifiDevice(QObject *parent)
    : NetworkDevice(DeviceType::Wifi, parent) {};
...
void WifiDevice::networkAdded(WifiNetwork *net) {
    this->mNetworks.insertObject(net);
}

void WifiDevice::networkRemoved(WifiNetwork *net) {
    this->mNetworks.removeObject(net);
}
