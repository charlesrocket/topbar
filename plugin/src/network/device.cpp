...
namespace {
Q_LOGGING_CATEGORY(logNetworkDevice, "topbar.network.device", QtInfoMsg);
} // namespace
...
void NetworkDevice::disconnect() {
    if (this->bState == ConnectionState::Disconnected) {
        qCCritical(logNetworkDevice)
            << "Device" << this << "is already disconnected";
        return;
    }

    if (this->bState == ConnectionState::Disconnecting) {
        qCCritical(logNetworkDevice)
            << "Device" << this << "is already disconnecting";
        return;
    }
