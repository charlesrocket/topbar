...
FreeBSDBackend::FreeBSDBackend(QObject *parent)
    ...
    QMetaObject::invokeMethod(
        this, &FreeBSDBackend::scanExistingDevices, Qt::QueuedConnection
    );
}
...
bool FreeBSDBackend::isAvailable() const {
    ...
    if (!available) {
        ...
        qCWarning(logNetworkFreeBSD)
            << "FreeBSD network backend is not available";
    }
...
void FreeBSDBackend::initializeRouteSocket() {
    ...
    QObject::connect(
        this->mRouteNotifier, &QSocketNotifier::activated, this,
        &FreeBSDBackend::onRouteSocketActivated
    );

    qCInfo(logNetworkFreeBSD)
...
void FreeBSDBackend::handleRouteMessage(const char *buf, ssize_t len) {
    ...
    if (rtm->rtm_type == RTM_IFANNOUNCE) {
        if (len < static_cast<ssize_t>(sizeof(struct if_announcemsghdr))) {
            qCWarning(logNetworkFreeBSD)
                << "RTM_IFANNOUNCE message is too small";
            return;
        }
    ...
    // RTM_IFINFO messages indicate interface state changes
        ...
        if (if_indextoname(ifm->ifm_index, ifnameBuf.data())) {
            ...
            if (!isIgnoredInterface(ifname)
                && this->mDevices.contains(ifname)) {
                qCDebug(logNetworkFreeBSD)
                    << "Interface state changed:" << ifname;

                if (auto *wifiDev =
                        qobject_cast<FreeBSDWifiDevice *>(this->mDevices[ifname]
                        )) {
                    wifiDev->handleInterfaceEvent();
                } else if (auto *wiredDev = qobject_cast<FreeBSDWiredDevice *>(
                               this->mDevices[ifname]
                           )) {
...
void FreeBSDBackend::scanExistingDevices() {
    ...
    QObject::connect(
        ...
        [this, process](int exitCode, QProcess::ExitStatus) {
            ...
            qCInfo(logNetworkFreeBSD)
                << "Scanning existing interfaces:" << interfaces;

            for (const QString &iface : interfaces) {
                if (isIgnoredInterface(iface)) {
                    qCDebug(logNetworkFreeBSD)
                        << "Skipping ignored interface:" << iface;
                    continue;
                }
            ...
            qCInfo(logNetworkFreeBSD)
                << "Total device count is" << this->mDevices.size();
...
void FreeBSDBackend::processInterface(
    const QString &interfaceName, bool isNew
) {
    if (this->mDevices.contains(interfaceName)) {
        if (!isNew) {
            qCDebug(logNetworkFreeBSD)
                << "Updating existing device:" << interfaceName;

            if (auto *wifiDev = qobject_cast<FreeBSDWifiDevice *>(
                    this->mDevices[interfaceName]
                )) {
                wifiDev->handleInterfaceEvent();
            } else if (auto *wiredDev = qobject_cast<FreeBSDWiredDevice *>(
                           this->mDevices[interfaceName]
            ...
            qCDebug(logNetworkFreeBSD)
                << "Device already exists:" << interfaceName;
        }
    ...
    if (isWirelessInterface(interfaceName)) {
        ...
        qCDebug(logNetworkFreeBSD)
            << "Skipping unknown interface type" << interfaceName;
    }
}
...
void FreeBSDBackend::setWifiEnabled(bool enabled) {
    ...
    for (auto *device : this->mDevices) {
        ...
        process->start(
            "ifconfig", QStringList()
                            << device->name() << (enabled ? "up" : "down")
        );
    }
...
FreeBSDWiredDevice::FreeBSDWiredDevice(
    const QString &interfaceName, QObject *parent
)
    : NetworkDevice(DeviceType::None, parent), mInterfaceName(interfaceName) {
    ...
    QObject::connect(
        this, &NetworkDevice::requestDisconnect, this,
        &FreeBSDWiredDevice::onDisconnectRequested
    );

    QObject::connect(
        this, &NetworkDevice::requestSetAutoconnect, this,
...
void FreeBSDWiredDevice::updateName(const QString &name) {
    this->bindableName().setValue(name);
}
...
void FreeBSDWiredDevice::handleInterfaceEvent() {
    qCDebug(logNetworkFreeBSD)
        << "Handling interface event for" << this->mInterfaceName;
    this->updateStateFromIfconfig();
}
...
void FreeBSDWiredDevice::parseIfconfigOutput(const QString &output) {
    ...
    if (match.hasMatch()) { this->updateAddress(match.captured(1)); }
    ...
    // Consider connected when an interface is UP and either has carrier or
    // shows active
...
void FreeBSDWiredDevice::onSetAutoconnectRequested(bool autoconnect) {
    ...
    qCInfo(logNetworkFreeBSD)
        << "Autoconnect for" << this->mInterfaceName << "set to" << autoconnect;
...
FreeBSDWifiDevice::FreeBSDWifiDevice(
    ...
    : WifiDevice(parent), mInterfaceName(interfaceName) {

    qCInfo(logNetworkFreeBSD)
        << "Creating wireless device for" << interfaceName;
    ...
    if (process.waitForFinished(3000)) {
        ...
        // Extract SSID
        const QRegularExpression ssidRegex(
            R"regex(ssid\s+(?:"([^"]+)"|(\S+)))regex"
        );
        match = ssidRegex.match(output);

        if (match.hasMatch()) {
            this->mCurrentSsid = match.captured(1).isEmpty()
                                   ? match.captured(2)
                                   : match.captured(1);
        }
    ...
    QObject::connect(
        this, &WifiDevice::scannerEnabledChanged, this,
        [this](bool enabled) {
...
void FreeBSDWifiDevice::updateName(const QString &name) {
    this->bindableName().setValue(name);
}
...
void FreeBSDWifiDevice::loadKnownNetworks() {
    ...
    QObject::connect(thread, &QThread::started, worker, [worker, ifaceName]() {
        ...
        const QStringList configPaths = {
            "/etc/wpa_supplicant.conf", "/usr/local/etc/wpa_supplicant.conf"
        };
        ...
        for (const QString &configPath : configPaths) {
            ...
            if (!configFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
                qCWarning(logNetworkFreeBSD)
                    << "Failed to open config file:" << configPath;
                continue;
            }
            ...
            while (!in.atEnd()) {
                ...
                if (line == "network={") {
                    ...
                    if (!currentSsid.isEmpty()) {
                        ...
                        qCDebug(logNetworkFreeBSD)
                            << "Found known network:" << currentSsid;
                    }
                    ...
                    if (ssidValue.startsWith('"') && ssidValue.endsWith('"')) {
                        if (ssidValue.length() >= 2) {
                            ssidValue =
                                ssidValue.mid(1, ssidValue.length() - 2);
                            ssidValue.replace(R"(\")", "\"");
                            ssidValue.replace(R"(\\)", "\\");
                        } else {
                            qCWarning(logNetworkFreeBSD)
                                << "Invalid quoted SSID (too short)";
                            continue;
                        }
                    } else if (ssidValue.startsWith("P\"")) {
                        qCDebug(logNetworkFreeBSD)
                            << "Skipping P-string SSID format:" << ssidValue;
                        continue;
                    }
                ...
                if (line.startsWith(ifconfigKey)) {
                    ...
                    if (match.hasMatch()) {
                        ...
                        if (match.lastCapturedIndex() >= 1) {
                            // captured(1) is quoted SSID, captured(2) is
                            // unquoted
                            ssid = match.captured(1).isEmpty()
                                     ? (match.lastCapturedIndex() >= 2
                                            ? match.captured(2)
                                            : QString())
                                     : match.captured(1);
                        }
                        ...
                        if (!ssid.isEmpty()) {
                            ...
                            qCDebug(logNetworkFreeBSD)
                                << "Found known network in rc.conf:" << ssid;
                        }
                    }
                }
            }
        ...
        worker->setProperty(
            "knownNetworks", QVariant::fromValue(knownNetworks)
        );
        worker->thread()->quit();
    });
    ...
    QObject::connect(
        ...
        [this, worker, thread]() {
            auto knownNetworks =
                worker->property("knownNetworks").value<QSet<QString>>();
            ...
            for (const QString &ssid : knownNetworks) {
                for (auto *network : this->mNetworkMap) {
                    if (network->name() == ssid) { network->updateKnown(true); }
                }
            }
    ...
    connect(
        ...
        [this, wpaCliProcess](int exitCode, QProcess::ExitStatus) {
            if (exitCode == 0) {
                // Skip header line
                const QString output =
                    QString::fromUtf8(wpaCliProcess->readAllStandardOutput());
                        ...
                        if (!ssid.isEmpty()) {
                            for (auto *network : this->mNetworkMap) {
                                if (network->name() == ssid) {
                                    qCDebug(logNetworkFreeBSD)
                                        << "Found known network via wpa_cli:"
                                        << ssid;
    ...
    wpaCliProcess->start(
        "wpa_cli", QStringList()
                       << "-i" << this->mInterfaceName << "list_networks"
    );
}
...
void FreeBSDWifiDevice::triggerScan() {
    if (this->mScanPending) {
        qCDebug(logNetworkFreeBSD)
            << "Scan already pending for" << this->mInterfaceName;
        return;
    }
    ...
    QObject::connect(
        ...
        [this, ifconfigProcess](int exitCode, QProcess::ExitStatus) {
            if (exitCode == 0) {
                const QString output =
                    QString::fromUtf8(ifconfigProcess->readAllStandardOutput());
                const QRegularExpression bssidRegex(
                    R"(bssid\s+([0-9a-f:]{17}))",
                    QRegularExpression::CaseInsensitiveOption
                );
                ...
                [this, scanRequestProcess](int, QProcess::ExitStatus) {
                    ...
                    QTimer::singleShot(1500, this, [this]() {
                            ...
                            [this, scanResultProcess](
                                ...
                                if (exitCode != 0) {
                                    ...
                                    qCWarning(logNetworkFreeBSD)
                                        << "Scan failed for"
                                        << this->mInterfaceName;
                                    this->mPendingProcesses.removeOne(
                                        scanResultProcess
                                    );
                                ...
                                const QString output = QString::fromUtf8(
                                    scanResultProcess->readAllStandardOutput()
                                );
                                this->mPendingProcesses.removeOne(
                                    scanResultProcess
                                );
                        ...
                        scanResultProcess->start(
                            "ifconfig", QStringList() << this->mInterfaceName
                                                      << "list" << "scan"
                        );
                    });
                }
            ...
            scanRequestProcess->start(
                "ifconfig", QStringList() << this->mInterfaceName << "scan"
            );
        }
    );
...
void FreeBSDWifiDevice::processScanLines(
    const QStringList &lines, int64_t startIndex
) {
    ...
    if (startIndex == 0) {
        if (this->mCurrentBssid.isEmpty() && isCurrentlyConnected) {
            qCWarning(logNetworkFreeBSD)
                << "WARNING: Device is connected but mCurrentBssid is EMPTY!";
            qCWarning(logNetworkFreeBSD)
                << "This means BSSID was not extracted from ifconfig output!";
        }
    }
    ...
    for (int64_t i = startIndex; i < endIndex; ++i) {
        ...
        const QRegularExpression bssidRegex(
            "([0-9a-f]{2}:[0-9a-f]{2}:[0-9a-f]{2}:[0-9a-f]{2}:[0-9a-f]{2}:[0-"
            "9a-f]{2})",
            QRegularExpression::CaseInsensitiveOption
        );
        ...
        const qsizetype afterBssidStart =
            bssidPos + bssidMatch.captured(1).length();

        if (afterBssidStart > trimmedLine.length()) continue;

        const QString afterBssid = trimmedLine.mid(afterBssidStart).trimmed();
        QStringList fields =
            afterBssid.split(QRegularExpression("\\s+"), Qt::SkipEmptyParts);
        ...
        if (this->mNetworkMap.contains(bssid)) {
            ...
            network =
                new FreeBSDWifiNetwork(ssid, bssid, this->mInterfaceName, this);
            isNewNetwork = true;
        }
        ...
        qCDebug(logNetworkFreeBSD)
            ...
            << "| Strength:" << qRound(strength * 100) << "%"
            << "| Security:" << static_cast<int>(security)
            << "| CAPS:" << capsString;

        // Check if this specific BSSID is currently connected
        const bool isConnectedBssid =
            (!this->mCurrentBssid.isEmpty() && bssid == this->mCurrentBssid
    ...
    if (endIndex < lines.size()) {
        ...
        qCDebug(logNetworkFreeBSD)
            << "Scan processing completed for" << this->mInterfaceName;
    }
}

void FreeBSDWifiDevice::onScanTimerTimeout() {
    if (this->scannerEnabled() && !this->mScanPending) { this->triggerScan(); }
}
...
void FreeBSDWifiDevice::handleScanComplete() {
    if (this->scannerEnabled()) { this->triggerScan(); }
}
...
void FreeBSDWifiDevice::parseIfconfigOutput(const QString &output) {
    ...
    const QRegularExpression bssidRegex(
        R"(bssid\s+([0-9a-f:]{17}))", QRegularExpression::CaseInsensitiveOption
    );
    ...
    const bool isUp = output.contains("status: active")
                   || output.contains("status: associated");
    this->updateConnectionState(
    ...
    for (auto it = this->mNetworkMap.begin(); it != this->mNetworkMap.end();
         ++it) {
        const QString &networkBssid = it.key(); // BSSID is the map key
        auto *network = it.value();

        const bool isMatch = isUp && !this->mCurrentBssid.isEmpty()
                          && (networkBssid == this->mCurrentBssid);
...
FreeBSDWifiNetwork::FreeBSDWifiNetwork(
    QString ssid, QString bssid, QString interfaceName, QObject *parent
)
    : WifiNetwork(std::move(ssid), parent), mBssid(std::move(bssid)),
      mInterfaceName(std::move(interfaceName)) {
    QObject::connect(
        this, &WifiNetwork::requestConnect, this,
        &FreeBSDWifiNetwork::onConnectRequested
    );

    QObject::connect(
        this, &WifiNetwork::requestDisconnect, this,
        &FreeBSDWifiNetwork::onDisconnectRequested
    );

    QObject::connect(
        this, &WifiNetwork::requestForget, this,
        &FreeBSDWifiNetwork::onForgetRequested
    );
}
...
void FreeBSDWifiNetwork::updateKnown(bool known) {
    this->bindableKnown().setValue(known);
}
...
void FreeBSDWifiNetwork::onConnectRequested() {
    qCInfo(logNetworkFreeBSD)
        << "Connecting to" << this->name() << "BSSID:" << this->mBssid << "on"
        << this->mInterfaceName;
    ...
    process->start(
        "ifconfig", QStringList()
                        << this->mInterfaceName << "ssid" << this->name()
                        << "bssid" << this->mBssid << "up"
    );
}

void FreeBSDWifiNetwork::onDisconnectRequested() {
    qCInfo(logNetworkFreeBSD)
        << "Disconnecting from" << this->name() << "BSSID:" << this->mBssid;
