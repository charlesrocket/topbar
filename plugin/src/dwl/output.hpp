...
class DwlIpcOutput : public QObject, public QtWayland::zdwl_ipc_output_v2 {
    Q_OBJECT;
    QML_ELEMENT;
    ...
    Q_PROPERTY(QList<topbar::dwl::DwlTag *> tags READ tags NOTIFY tagsChanged);
    /// Keyboard layout.
    Q_PROPERTY(QString kbLayout READ kbLayout NOTIFY kbLayoutChanged);

  public:
    explicit DwlIpcOutput(
        ::zdwl_ipc_output_v2 *handle, QString name, QObject *parent = nullptr
    );
  ...
  signals:
    ...
    void tagsChanged();
    /// Emitted when all double-buffered state for this frame has been
    /// committed.
    void frame();
