...
DwlIpcOutput::DwlIpcOutput(
    ::zdwl_ipc_output_v2 *handle, QString name, QObject *parent
)
    : QObject(parent), QtWayland::zdwl_ipc_output_v2(handle),
...
void DwlIpcOutput::zdwl_ipc_output_v2_toggle_visibility() {
...
void DwlIpcOutput::zdwl_ipc_output_v2_tag(
    ...
    if (std::cmp_less(tag, this->mTags.size()))
        this->mTags[static_cast<qsizetype>(tag)]->updateState(
