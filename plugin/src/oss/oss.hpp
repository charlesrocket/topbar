...
Q_DECLARE_LOGGING_CATEGORY(logOSS)
...
class OSSMixerControl : public QObject {
  ...
  public:
    explicit OSSMixerControl(
        QString name, int devId, int mixerId, MixerControlType type,
...
/// Uses devd for real-time device change notifications. Headphones/jack
/// detection is done via kqueue (/var/log/messages).
///
...
Q_DECLARE_METATYPE(topbar::oss::DeviceMode);
Q_DECLARE_METATYPE(topbar::oss::MixerControlType);
