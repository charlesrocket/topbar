...
OSSMixerControl::OSSMixerControl(
    QString name, int devId, int mixerId, MixerControlType type, QObject *parent
)
    : QObject(parent), mName(std::move(name)), mDeviceId(devId),
      mMixerId(mixerId), mType(type) {
    this->refresh();
}
...
bool OSSMixerControl::writeValue() {
    ...
    if (fd < 0) {
        qCWarning(logOSS) << "Failed to open mixer device for writing:"
                          << mixerPath;
        return false;
    }
...
OSSSoundDevice::OSSSoundDevice(int deviceId, QObject *parent)
    : QObject(parent), mDeviceId(deviceId), mDescription(this->mName) {
    ...
    if (sndstat.open(QIODevice::ReadOnly | QIODevice::Text)) {
        ...
        while (in.readLineInto(&line)) {
            if (line.contains(QString("pcm%1:").arg(this->mDeviceId))) {
                ...
                if (line.contains("default")) { this->mIsDefault = true; }
...
void OSSSoundDevice::loadControls() {
    ...
    for (auto i = 0; i < numDevs; i++) {
        if (devmask & (1 << i)) {
            ...
            auto *control =
                new OSSMixerControl(name, this->mDeviceId, i, type, this);
...
void OSSSoundDevice::refresh() {
    for (auto *control : this->mControls) { control->refresh(); }
}
...
QList<QObject *> OSSSoundDevice::controls() const {
    ...
    for (auto *control : this->mControls) { list.append(control); }
...
QList<QObject *> OSS::devices() const {
    ...
    for (auto *device : this->mDevices) { list.append(device); }
...
void OSS::setupJackDetection() {
    ...
    if (this->mLogFileDescriptor < 0) {
        qCWarning(logOSS
        ) << "Failed to open /var/log/messages for jack detection";
        return;
    }

    // Seek to end to only catch new messages
    this->mLogFilePosition =
        lseek(this->mLogFileDescriptor, 0, SEEK_END); // NOLINT
    ...
    this->mKqueueNotifier =
        new QSocketNotifier(this->mKqueue, QSocketNotifier::Read, this);
    connect(
        this->mKqueueNotifier, &QSocketNotifier::activated, this,
        &OSS::handleKqueueEvent
    );
...
void OSS::syncJackProperties() {
    for (auto it = this->mJackStates.constBegin();
         it != this->mJackStates.constEnd(); ++it) {
        ...
        if (output.contains("Headphones", Qt::CaseInsensitive)) {
            if (this->mHeadphonesConnected != connected) {
                ...
                qCInfo(logOSS) << "Initial headphone state:"
                               << (connected ? "connected" : "disconnected");
                emit this->headphonesChanged(connected);
            }
        }
...
void OSS::initializeJackStates() {
    ...
    for (const QString &line : lines) {
        // Look for jack pins
        if (line.contains("dev.hdaa.") && line.contains(": pin:")
            && line.contains("Jack")) {
            const QRegularExpression regex(
                R"(dev\.hdaa\.\d+\.nid(\d+):\s+pin:\s+(.+))"
            );
            ...
            if (match.hasMatch()) {
                ...
                qCDebug(logOSS)
                    << "Found jack: nid=" << nid << "desc=" << pinDesc;
...
void OSS::readNewLogLines() {
    QFile logFile;
    if (!logFile.open(
            this->mLogFileDescriptor, QIODevice::ReadOnly | QIODevice::Text,
...
void OSS::readRecentLogMessages() {
    ...
    if (size > 8192) { logFile.seek(size - 8192); }
    ...
    for (const QString &line : lines) {
        if (!line.contains("Pin sense:")) { continue; }

        const QRegularExpression nidRegex(R"(nid=(\d+))");
        auto nidMatch = nidRegex.match(line);

        if (!nidMatch.hasMatch()) { continue; }
    ...
    for (auto it = lastLoggedState.constBegin();
         it != lastLoggedState.constEnd(); ++it) {
        const int nid = it.key();
        const bool connected = it.value();
...
void OSS::refresh() {
    ...
    for (auto *device : this->mDevices) { device->refresh(); }
}
